 ...
 */
{
  ...
  /* Allocation of space for the nb of matrix rows and columns */
  {
    ...
    size_t* nb_rows = (size_t*) Mry_New(size_t,2*n) ;
    size_t* nb_cols = nb_rows + n ;
...
size_t  (Nodes_UpdateTheNbOfUnknownsAndEquationsPerNode)(Nodes_t* nodes)
/** Update 
 ...
 */
{
  ...
  {  
    for(size_t in = 0 ; in < n_no ; in++) {
      ...
      if(Node_GetNbOfUnknowns(node_i) != Node_GetNbOfEquations(node_i)) {
        size_t ind = Node_GetNodeIndex(node_i) ;
        ...
        arret("Nodes_UpdateTheNbOfUnknownsAndEquationsPerNode: nb of unknowns and equations not equal at node %lu",ind) ;
      }
    }
  }
  ...
  /* The total nb of degrees of freedom */
  {
    ...
    /* Number of unknowns */
    for(size_t i = 0 ; i < n_no ; i++) {
      n_inc += (size_t) Node_GetNbOfUnknowns(node + i) ;
    }
  
    /* Number of equations */
    for(size_t i = 0 ; i < n_no ; i++) {
      n_equ += (size_t) Node_GetNbOfEquations(node + i) ;
    }
...
/** Set up the system */
{
  ...
  size_t* nbofmatrixrows = Nodes_GetNbOfMatrixRows(nodes) ;
  size_t* nbofmatrixcols = Nodes_GetNbOfMatrixColumns(nodes) ;
      ...
      for(j = 0 ; j < Node_GetNbOfUnknowns(node_i) ; j++) {
        ...
        if(icol >= 0) {
          ...
          if(seq_ind >= 0) {
            ...
            Node_GetMatrixColumnIndex(node_i)[j] = (int) nbofmatrixcols[imatrix] ;
      ...
      for(j = 0 ; j < Node_GetNbOfEquations(node_i) ; j++) {
        ...
        if(irow >= 0) {
          ...
          if(seq_ind >= 0) {
            ...
            Node_GetMatrixRowIndex(node_i)[j] = (int) nbofmatrixrows[imatrix] ;
 ...
 **/
{
  ...
  size_t n_nodes = Nodes_GetNbOfNodes(nodes) ;
  Node_t* node = Nodes_GetNode(nodes) ;
