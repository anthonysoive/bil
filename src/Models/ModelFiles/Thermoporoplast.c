 ...
 * (vim and vim_n must be used as pointer in the remaining file) */
#define M_L           (vim   + 0)[0]
...
#define S_TOT         (vim   + 27)[0]
#define S_TOTn        (vim_n + 27)[0]

#define W_THE         (vim   + 28)

#define PHI           (vim   + 31)[0]
#define PHI_n         (vim_n + 31)[0]


/* We define some names for explicit terms
 * (vex must be used as pointer in the remaining file) */
#define K_L           (vex + 0)
#define K_T           (vex + 3)


/* We define some names for constant terms
 * (vc must be used as pointer in the remaining file) */
#define SIG0          (vc + 0)      //initial stresses
#define P_L0          (vc + 9)[0]   //initial porepressure
#define T0            (vc + 10)[0]  //initial temperature
...
static double  visc_w(double,double) ;
static double  dens_w(double,double) ;
...
#define  SetElasticModel(I) \
         do { \
           if(elasticmodel < 0) { \
             elasticmodel = I ; \
           } else if(elasticmodel != I) { \
             Message_FatalError("Incompatible model") ; \
           } \
         } while(0)
  ...
  /* Elastic and plastic properties */
  {
    ...
    
    {
      /* Elasticity */
      {
        ...
        double young_3   = Material_GetProperty(mat)[pm("young_3")] ;
        double poisson_3 = Material_GetProperty(mat)[pm("poisson_3")] ;
        double shear_3   = Material_GetProperty(mat)[pm("shear_3")] ;
        double axis_3    = Material_GetProperty(mat)[pm("axis_3")] - 1 ;
 ...
 *  */
{
  ...
  /* Needed variables to compute secondary variables */
  {
    ...
    /* Temperature at previous time step (u_n) */
    x_n[I_TEM] = FEM_ComputeUnknown(fem,u_n,intfct,p,U_tem) ;
...
void  ComputeSecondaryVariables(Element_t* el,double t,double dt,double* x_n,double* x)
{
  ...
  double phi_n = x_n[I_PHI] ; //ADDED
  double rho_l = x[I_RHO_L] ; //ADDED


  /* Temperature */  //ADDED
  double tem    = x[I_TEM] ;  
  double tem_n  = x_n[I_TEM] ;
  double tem_0  = T0 ; 
  ...
  /* Backup stresses, plastic strains */
  {
    ...
    double pc_n = x_n[I_HARDV] ; //ADDED    //!    MAYBE SWITCH TO I_HARDV
...
//#define dens_w(T,p)     (wf1[0]+wf1[1]*p*1.e-6+wf1[2]*T+wf1[3]*p*1.e-6*p*1.e-6+wf1[4]*p*1.e-6*T+wf1[5]*T*T+wf1[6]*p*1.e-6*p*1.e-6*T+wf1[7]*p*1.e-6*T*T+wf1[8]*T*T*T)


double dens_w(double T,double p)
/** Inputs:
 *  T in C
 *  p in Pa
 * 
...
{
  double wf1[] = {1.00E+03,4.87E-01,-2.84E-02,-3.95E-04,-1.82E-03,-0.004974,-0.000001122,0.00001615,0.00001032};
  double rho_w = (wf1[0]+wf1[1]*p*1.e-6+wf1[2]*T+wf1[3]*p*1.e-6*p*1.e-6+wf1[4]*p*1.e-6*T+wf1[5]*T*T+wf1[6]*p*1.e-6*p*1.e-6*T+wf1[7]*p*1.e-6*T*T+wf1[8]*T*T*T) ;
...
double visc_w(double T,double p)
{
  double wf6[] = {1.63E-03,    -1.11E-06,    -3.83E-05,    3.92E-09,    3.64E-08,    4.123E-07,    -4.878E-11,    -2.316E-10,    -1.652E-09};
  double nu_w = ((wf6[0]+wf6[1]*p*1.e-6+wf6[2]*T+wf6[3]*p*1.e-6*p*1.e-6+wf6[4]*p*1.e-6*T+wf6[5]*T*T+wf6[6]*p*1.e-6*p*1.e-6*T+wf6[7]*p*1.e-6*T*T+wf6[8]*T*T*T)) ;
...
double enth_w(double T,double p)
{
  double wf2[] = {1.56E-03,    -1.91E-05,    1.53E-02,    -9.65E-07,    -9.90E-06,    -0.00002634,    1.972E-08,    2.318E-08,    4.362E-08};
  double s_w = ((wf2[0]+wf2[1]*p*1.e-6+wf2[2]*T+wf2[3]*p*1.e-6*p*1.e-6+wf2[4]*p*1.e-6*T+wf2[5]*T*T+wf2[6]*p*1.e-6*p*1.e-6*T+wf2[7]*p*1.e-6*T*T+wf2[8]*T*T*T)*1000) ;
...
double bulk_w(double T,double p)
{
  double wf3[] = {1.975,    5.34E-03,    1.37E-02,    6.79E-06,    6.98E-07,    -0.0001817,    -6.954E-08,    1.331E-07,    5.115E-07};
  double k_w = ((wf3[0]+wf3[1]*p*1.e-6+wf3[2]*T+wf3[3]*p*1.e-6*p*1.e-6+wf3[4]*p*1.e-6*T+wf3[5]*T*T+wf3[6]*p*1.e-6*p*1.e-6*T+wf3[7]*p*1.e-6*T*T+wf3[8]*T*T*T)*1000*1.e6) ;
...
double alpha_w(double T,double p)
{
  double wf4[] = {-3.03E-05,    2.90E-06,    1.33E-05,    -5.70E-09,    -7.33E-08,    -8.702E-08,    1.238E-10,    2.771E-10,    3.188E-10};
  double a_w = (wf4[0]+wf4[1]*p*1.e-6+wf4[2]*T+wf4[3]*p*1.e-6*p*1.e-6+wf4[4]*p*1.e-6*T+wf4[5]*T*T+wf4[6]*p*1.e-6*p*1.e-6*T+wf4[7]*p*1.e-6*T*T+wf4[8]*T*T*T) ;
