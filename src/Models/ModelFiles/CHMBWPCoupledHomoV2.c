...
#define Diffusion_Coupled_homogenization_MT(n,tau,rho_l,tau0,Q,w_s) \
        (((1.+2.*(n))/(1.-(n)))*(1.-((tau)*(tau_sat))/(tau0))*(nu*(w_s)*Q/(1.-(w_s)))*(rho_w_bit*d_w_bit*M_w)/(Molarm_s*(rho_l)))
// for coupled homogenization 
...

  {
    ...
    double testtre = 0.;    //debug 

      {
        ...
        testtre = eps[0] + eps[4] + eps[8] ; //for debug 
...
void  ComputeSecondaryVariables(Element_t* el,double dt,double* x_n,double* x)
{
  ...
  //~ }
    ...
    if (phi_l < 0) {
      Message_Direct("\nNegative porosity: %e\n",phi_l) ;
      printf("\n CoorX = %.5E | phi_l_n = %.5E | n_n = %.5E | biot = %.5E | biot_v = %.5E | dtre = %.5E | dp/Nbiot = %.5E| dphi_l = %.5E |tre = %.5E | tre_n = %.5E | dphi_l_salt = %.5E | phi_c = %.5E | phi_c_n = %.5E | w_s = %.5E | w_s_n = %.5E | n = %.5E",coorX, phi_l_n, n_n, biot,biot_v, tre - tre_n, (1 + tre)*(p - p_n)/Nbiot, dphi_l, tre, tre_n, dphi_l_salt, phi_c, phi_c_n, w_s, w_s_n, n);
      Exception_Interrupt ;
    }
