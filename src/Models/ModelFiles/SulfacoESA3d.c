...
/* ATTENTION: the 5 first equations must be related to the diffusion equations */
enum {
  E_Sulfur,
  E_charge,
  E_Calcium,
...
/* Nb of diffusion equations */
#define NbOfDiffusionEquations        (E_Aluminium + 1)

#define NEQ    NbOfEquations
#define NDIF   NbOfDiffusionEquations
...
#define P_C          (vi   + 54)[0]
#define P_Cn         (vi_n + 54)[0]




/* We define some names for explicit terms (ve must be used as pointer below) */
#define NVE   (1 + CementSolutionDiffusion_NbOfConcentrations + NDIF*NDIF)     /*  nb of explicit terms per point */
#define TORTUOSITY       (ve)[0]

#define CONCENTRATION    (ve + 1) // CementSolutionDiffusion_NbOfConcentrations

#define KFick            (ve + 1 + CementSolutionDiffusion_NbOfConcentrations) // NDIF*NDIF terms
...
#define InverseOfCrystallizationPressure(pc) \
        (exp(V_Crystal/RT*(pc)))
#define dInverseOfCrystallizationPressure(pc) \
        (V_Crystal/RT*exp(V_Crystal/RT*(pc)))
...
static void    ComputeFluxes(double*,double*,double*) ;
...
static double SolveCrystalGrowthKineticsForCrystallizationPressure(double,double,double,double,double,double,double) ;
static double SolveCrystalGrowthKineticsForPoreRadius(double,double,double) ;
static double CrystalPoreDeformation(double,double,double,double) ;
...
#define CopyElasticStiffnessTensor(...)      Damage_CopyElasticStiffnessTensor(damage,__VA_ARGS__)
...
I_CHEMICALPOTENTIAL_end = I_CHEMICALPOTENTIAL + CementSolutionDiffusion_NbOfConcentrations - 1,

I_KFick,

I_KFick_end = I_KFick + NDIF*NDIF - 1,
 ...
 */ 
{
  ...
  */
  ...
  Element_CopyCurrentSolutionIntoPreviousSolution(el) ;
  
  /* Initialize the implicit terms */
  ComputeImplicitTerms(el,0,0) ;
  
  /* Initialize the explicit terms */  
  ComputeExplicitTerms(el,0) ;
 ...
 *  Return 0 if succeeded and -1 if failed */
{
  ...
  */
...

  {
    ...
    for(p = 0 ; p < np ; p++) {
      double* ve  = ve0 + p*NVE ;
      double* xp  = ComputeVariables(el,u,u,f,0,0,p) ;
      ...
      /* The coupled diffusion matrix related to elements: S,q,Ca,K,Al.
       * The indexes of diffusion equations are assumed to range from 0 to NDIF-1 */
      {
        int idif ;
        int I_U[NDIF] ;
        
        I_U[E_Sulfur]    = I_U_Sulfur ;
        I_U[E_charge]    = I_U_charge ;
        I_U[E_Calcium]   = I_U_Calcium ;
        I_U[E_Potassium] = I_U_Potassium ;
        I_U[E_Aluminium] = I_U_Aluminium ;
        
        for(idif = 0 ; idif < NDIF ; idif++) {
          double  duk  = dui[idif] ;
          int     iuk  = I_U[idif] ;
          double* dxp = ComputeVariableDerivatives(el,0,0,xp,duk,iuk) ;
          double* g = CementSolutionDiffusion_GetGradient(csd) ;
          double* c = CONCENTRATION ;
          double* dmu = dxp + I_CHEMICALPOTENTIAL ;
          double tortuosity = TORTUOSITY ;
          int n = CementSolutionDiffusion_NbOfConcentrations ;
        
          /* The coefficient relative to the gradient of unknown idif */
          {
            int l ;
          
            for(l = 0 ; l < n ; l++) {
              g[l] = tortuosity * c[l] * dmu[l] ;
            }
          }
        
          CementSolutionDiffusion_ComputeFluxes(csd) ;
        
          #define KFICK(E)  (KFick + (E)*NDIF)[idif]
          {
            /* We change the sign to get the transport coefficients */
            KFICK(E_Calcium)   = - CementSolutionDiffusion_GetElementFluxOf(csd,Ca) ;
            KFICK(E_Sulfur)    = - CementSolutionDiffusion_GetElementFluxOf(csd,S) ;
            KFICK(E_Potassium) = - CementSolutionDiffusion_GetElementFluxOf(csd,K) ;
            KFICK(E_Aluminium) = - CementSolutionDiffusion_GetElementFluxOf(csd,Al) ;
            KFICK(E_charge)    = - CementSolutionDiffusion_GetIonCurrent(csd) ;
          }
          #undef KFICK
        }
  ...
  /* Loop on integration points */
  {
    ...
    for(p = 0 ; p < NbOfIntPoints ; p++) {
      ...
      /* storage in vi */
      {
        ...
        for(i = 0 ; i < 3 ; i++) W_q[i]  = x[I_W_q + i] ;
        for(i = 0 ; i < 3 ; i++) W_Si[i] = x[I_W_Si + i] ;
  ...
  */
  {
  ...
  
    {
      ...
      for(E_dif = 0 ; E_dif < ndif ; E_dif++) {
        int U_dif ;
        
        for(U_dif = 0 ; U_dif < ndif ; U_dif++) {
          double* c1 = c + 9*(E_dif*ndif + U_dif) ;
          double* kc = FEM_ComputeConductionMatrix(fem,intfct,c1,dec) ;
          int    i ;
          ...
          for(i = 0 ; i < nn ; i++) {
            ...
            for(j = 0 ; j < nn ; j++) {
              K(E_dif + i*NEQ,U_dif + j*NEQ) += dt*kc[i*nn + j] ;
            }
          }
...
/* Les valeurs exploitees (s) */
{
  ...
  */
  ...
  FEM_AddAverageTerms(fem,NVI,NVE,NV0) ;
...
*/
    ...
    Result_Store(r + i++,w_si,"W_Si",dim) ;
    Result_Store(r + i++,w_ca,"W_Ca",dim) ;
    Result_Store(r + i++,w_s ,"W_S",dim) ;
    Result_Store(r + i++,w_al,"W_Al",dim) ;
    Result_Store(r + i++,w_q ,"W_q",dim) ;
...
*/
{
...

  {
    ...
    for(p = 0 ; p < np ; p++) {
      ...
      /* The coupled diffusion matrix of elements.
       * We assume that the indexes of the diffusion equations range from 0 to NDIF-1 */
      {
        int i ;
        
        for(i = 0 ; i < NDIF*NDIF ; i++) {
          double* c2 = c1 + 9*i ;
        ...
        /* Tangent stiffness matrix */
        {
          ...
          /* Tangent stiffness matrix */
          {
            ...
            if(crit >= 0.) {
              ...
              double d = Damage ;
              double hardv = Hardv ;
              double crit1 = ComputeTangentStiffnessTensor(eps,&d,&hardv) ;
...
{
  int dim = Element_GetDimensionOfSpace(el) ;
  /* Primary variables at the current time */
  double zn_si_s    = 1 ;
   ...
   * ============================== */
  {
      /* Projection */
      {
        ...
        double crit  = ReturnMapping(strain,&d,&hardv) ;
   ...
   * ====================== */
  {
    ...
    /* backup */
    {
      double* mu = x + I_CHEMICALPOTENTIAL ;
      
      HardenedCementChemistry_CopyChemicalPotential(hcc,mu) ;
    }
  }
  ...
  /* Crystal growth kinetics dealing with the saturation degree of crystal.
   * Solve the kinetic equation for the radius r.
   * ====================================================================== */
  {
    /* The saturation index of AFt */
    double beta  = HardenedCementChemistry_GetSaturationIndexOf(hcc,AFt) ;
    double r_n = x_n[I_PoreRadius] ;
    double r   = SolveCrystalGrowthKineticsForPoreRadius(r_n,beta,dt) ;
  ...
  /* Crystal growth kinetics dealing with the crystal pore deformation.
   * Solve the kinetic equation for the crystallization pressure p_c.
   * ==================================================================*/
  {
    /* The saturation index of AFt */
    double beta  = HardenedCementChemistry_GetSaturationIndexOf(hcc,AFt) ;
    ...
    #else
      double p_cn = x_n[I_P_C] ;
      double varphi_cn  = x_n[I_VarPHI_C] ;
      double p_c = SolveCrystalGrowthKineticsForCrystallizationPressure(p_cn,varphi_cn,strainv,s_c,beta,d,dt) ;
    #endif
    double beta_p = InverseOfCrystallizationPressure(p_c) ;
  
    /* Compute the crystal pore deformation */
    double varphi_c = CrystalPoreDeformation(p_c,strainv,s_c,d) ;
  ...
  {
    double s_c = x[I_S_C] ;
    double varphi_c = x[I_VarPHI_C] ;
    /* 1) as components: CH, CSH2, CSH, AH3, AFm, AFt, C3AH6
     * ----------------------------------------------------- */
    ...
    double n_csh      = CSHSolidContent(zn_si_s) ;
    double n_so4ads   = 0; //n_csh * AdsorbedSulfatePerUnitMoleOfCSH(c_so4,c_oh) ;
  
    /* 2) as elements: S, Ca, Si, Al
     * ----------------------------- */
  ...
  {
    /* due to chemistry */
    double v_cem      = x[I_V_Cem] ;
    double v_cem0     = x_n[I_V_Cem0] ;
    double phi_con    = phi0 + v_cem0 - v_cem ;

    /* and due to deformation */
    double s_c = x[I_S_C] ;
    double p_c = x[I_P_C] ;
    double d   = x[I_Damage] ;
    double bd  = Biot + d * (1 - Biot) ;
    double varphi = bd * strainv + (bd - phi0)*(1 - Biot)/K_bulk*p_c*s_c ;
  ...
  /* Diffusion fluxes */
  {
    double* kfick = x_n + I_KFick ;
    
    ComputeFluxes(x,kfick + E_Sulfur*NDIF   ,x + I_W_S) ;
    ComputeFluxes(x,kfick + E_Calcium*NDIF  ,x + I_W_Ca) ;
    ComputeFluxes(x,kfick + E_Potassium*NDIF,x + I_W_K) ;
    ComputeFluxes(x,kfick + E_charge*NDIF   ,x + I_W_q) ;
    ComputeFluxes(x,kfick + E_Aluminium*NDIF,x + I_W_Al) ;
  }
}



void ComputeFluxes(double* x,double* kf,double* w)
{
  double* grd_sulfur    = (x + I_GRD_U_Sulfur) ;
  double* grd_calcium   = (x + I_GRD_U_Calcium) ;
  double* grd_potassium = (x + I_GRD_U_Potassium) ;
  double* grd_charge    = (x + I_GRD_U_charge) ;
  double* grd_aluminium = (x + I_GRD_U_Aluminium) ;

  {
    ...
    for(i = 0 ; i < 3 ; i++) {
      w[i]  = 0 ;
      w[i] -= kf[E_Sulfur]    * grd_sulfur[i] ;
      w[i] -= kf[E_Calcium]   * grd_calcium[i] ;
      w[i] -= kf[E_Potassium] * grd_potassium[i] ;
      w[i] -= kf[E_charge]    * grd_charge[i] ;
      w[i] -= kf[E_Aluminium] * grd_aluminium[i] ;
    }
  }
}
...
double SolveCrystalGrowthKineticsForPoreRadius(double r_n,double beta,double dt)
{
  ...
  
  {
    ...
    for(i = 0 ; i < iterations ; i++) {
      ...
      if(i > 30) {
        ...
        Message_Direct("------------\n") ;
        Message_Direct("s_ln   = %e, ",s_ln) ;
        //Message_Direct("s_linf = %e, ",s_linf) ;
        Message_Direct("s_l    = %e\n",s_l) ;
        Message_Direct("ds_l   = %e, ",ds_l) ;
        Message_Direct("dbeta_i= %e\n",dbeta_i) ;
        Message_Direct("r_n    = %e, ",r_n) ;
        Message_Direct("r_inf  = %e, ",r_inf) ;
        Message_Direct("dr/r_n = %e, ",dr/r_n) ;
        Message_Direct("r      = %e\n",r) ;
        Message_Direct("beta_i = %e, ",beta_i) ;
        Message_Direct("beta   = %e\n",beta) ;
      }
    }
  }
  
  Message_Warning("SolveCrystalGrowthKineticsForPoreRadius: not converged") ;
  Exception_Interrupt ;
...
double SolveCrystalGrowthKineticsForCrystallizationPressure(double pc_n,double varphi_cn,double strainv,double sc,double beta,double d,double dt)
/** Solve the crystal growth kinetic equation for pc, namely 
 *  the kinetics of the crystal pore deformation.
 * 
 * On inputs:
 *   - sc = saturation degree of the crystal
 *   - pc_n = crystallization pressure at the previous time
 *   - strainv = volumetric strain
 *   - varphi_cn = the crystal pore deformation at the previous time
 *   - beta = saturation index of the crystal
 *   - dt = time increment
 * 
 * Return pc
 */
{
  ...
  
  {
    ...
    for(i = 0 ; i < iterations ; i++) {
      double varphi_c = CrystalPoreDeformation(pc,strainv,sc,d) ;
      double beta_p   = InverseOfCrystallizationPressure(pc) ;
      double phicrate = PoreCrystalGrowthRate(sc,beta,beta_p) ;
      
      /* Equation to solve for pc */
      double eq      = varphi_c - varphi_cn - dt * phicrate ;
      
      /* The derivative of equation wrt pc */
      double dvarphi_c = sc*(bd - phi0)/Kd*(1 - bd*sc) ;
      double dbeta_p   = dInverseOfCrystallizationPressure(pc) ;
      double dphicrate = dPoreCrystalGrowthRate(sc,beta,beta_p) * dbeta_p ;
      double deq       = dvarphi_c - dt * dphicrate ;
      
      /* Under-relaxation */
      double relax = 1 ;
      double dpc   = (fabs(deq) > 0) ? - relax * eq/deq : 0. ;
      
      /* pc cannot be negative */
      //if(dpc < - 0.5*pc) dpc = - 0.5*pc ;
      
      pc += dpc ;
      
      //if(fabs(dpc) < tol * fabs(pc)) {
      if(fabs(dpc*dvarphi_c) < tol) {
        return(pc) ;
      }
  ...
  Message_Direct("SolveCrystalGrowthKineticsForCrystallizationPressure\n") ;
  Message_Direct("no convergence!\n") ;
  Message_Direct("pc = %e\n",pc) ;
  Exception_Interrupt ;
...
double CrystalPoreDeformation(double pc,double strainv,double sc,double d)
/** Return the crystal pore deformation from the CE of poromechanics.
 * 
 *  On input:
 *   - sc = saturation degree of the crystal
 *   - pc = crystallization pressure
 *   - strainv = volumetric strain
 *   - d = damage variable
 * 
 *  Parameters:
 *   - Biot   = Biot's coefficient of undamaged material
 *   - K_bulk = Compression drained bulk modulus of undamaged material
 *   - phi0   = porosity
 */
{
  double bd = Biot + d * (1 - Biot) ;
  ...
  if(d < 1) {
    varphi_c = sc * (bd*strainv + (bd - phi0)/Kd*(1 - bd*sc)*pc) ;
  } else {
    Message_Direct("CrystalPoreDeformation:\n") ;
    Message_Direct("Damage variable d = %e\n",d) ;
    Exception_Interrupt ;
  }
