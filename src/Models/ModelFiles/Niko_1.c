...
#define COEFF(p)               Curve_ComputeValue(Element_GetCurve(el) + coeff,p)
...
#define DCOEFF(p)              Curve_ComputeDerivative(Element_GetCurve(el) + coeff,p)
...
/** Read the material properties in the stream file ficd */
{
  ...
  /* NOT USED UP TO NOW */
  {
    ...
    Curve_t *adpressure_curve = curve + 2 ;
...
/** Print the model characteristics */
{
  ...
  fprintf(ficd,"K_m = 2e8         # Compression modulus of the coal matrix\n") ;
  ...
  fprintf(ficd,"Curves = myfile   # p_co2 = Range{x1 = 0 , x2 = 1.e8 , n = 1001} n_ads = Langmuir(1){n_max = 2.03e3 , p0 = 1.08e6} rho = Redilch-Kwong_CO2(1){T = 293} ads_pressure = ? \n") ;
