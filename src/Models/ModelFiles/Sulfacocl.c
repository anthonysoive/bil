...
/**  Tangent matrix coefficients (c) */
{
  ...
  for(i = 0 ; i < nn ; i++) {
    ...
    #ifdef E_Chlorine
      #ifdef U_C_Cl
      dui[E_Chlorine ] =  1.e-3 * ObVal_GetValue(obval + E_Chlorine)/(Ln10*Un_Chlorine(i)) ;
      #endif
    #endif
...
void  ComputeSecondaryVariables(Element_t* el,double t,double dt,double* x_n,double* x)
{
  ...
  /* Solid contents
   * -------------- */
  /* ... as components: CH, CSH2, CSH, AH3, AFm, AFt, C3AH6, Friedel's salt */
